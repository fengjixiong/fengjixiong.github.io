...
void myQuit(void)
{
    ...
    mySipUA.flag = 0;
   	mySipUA.flag2=0;
    mySipUA.flag3=0;
	printf("I'm goting to set global quit!\n");
	gblSetQuit();
	printf("Set global quit!\n");
}
...
void reInviteCall()
{
	...
	mySipUA.payload_str = (char *)osip_list_get(&mySipUA.speech_md_req->m_payloads, 0);	//鍙栦富鍙獟浣撹兘鍔涚紪鐮?	if(!mySipUA.payload_str)
...
void reRegOK()
{
	...
	printf("reRegOK()\n");
	#endif
	if(0 < mySipUA.je->response->contacts.nb_elt)	/*bind 鏁板ぇ浜?琛ㄧず娉ㄥ唽锛屽惁鍒欎负娉ㄩ攢*/
	{
		printf("Registered!\n");
	}
	else
...
void *SipThread()
{
        ...
        while(mySipUA.flag2)
        {
			...
			printf("Act Sip thread!\n");
		}
	printf("Sip thread is quiting!\n");
